import { useState } from "react";
import { useConnect, type Connector } from "wagmi";

interface Props {
  onClose: () => void;
}

// ---------------------------------------------------------------------------
// Connector metadata for friendly display
// ---------------------------------------------------------------------------
interface ConnectorMeta {
  label: string;
  description: string;
  tag?: string;
  icon: string;
}

function getConnectorMeta(connector: Connector): ConnectorMeta {
  const name = connector.name.toLowerCase();

  if (name.includes("coinbase") && (connector as any).preference === "smartWalletOnly") {
    return {
      label: "Email or Phone",
      description: "Sign in with your email — no extension needed",
      tag: "Easiest",
      icon: "M",
    };
  }
  if (name.includes("coinbase")) {
    return {
      label: "Coinbase Wallet",
      description: "Use the Coinbase Wallet app or extension",
      icon: "C",
    };
  }
  if (name.includes("metamask")) {
    return {
      label: "MetaMask",
      description: "Connect using MetaMask browser extension",
      icon: "M",
    };
  }
  if (name.includes("walletconnect")) {
    return {
      label: "WalletConnect",
      description: "Scan a QR code with your mobile wallet",
      icon: "W",
    };
  }
  return {
    label: connector.name,
    description: "Connect with this wallet",
    icon: connector.name.charAt(0),
  };
}

// ---------------------------------------------------------------------------
// Education content — shown once to help Web2 users
// ---------------------------------------------------------------------------
const EDUCATION_STEPS = [
  {
    title: "What's a wallet?",
    body: "A digital wallet stores your money and lets you pay securely — like Apple Pay, but for crypto. You stay in control of your funds at all times.",
  },
  {
    title: "Is it safe?",
    body: "Your wallet is protected by your device's security (fingerprint, face ID, or password). Nobody can access your funds without your approval.",
  },
  {
    title: "Do I need crypto?",
    body: "You can start with email sign-in (Coinbase Smart Wallet). You'll be guided through adding funds when you're ready to buy.",
  },
];

/**
 * Full-screen modal for wallet connection.
 * - Education section for first-time users
 * - Connector list with clear descriptions
 * - Highlights the easiest option for Web2 users
 */
export default function ConnectModal({ onClose }: Props) {
  const { connect, connectors, isPending, error } = useConnect();
  const [showEducation, setShowEducation] = useState(false);
  const [connectingId, setConnectingId] = useState<string | null>(null);

  const handleConnect = async (connector: Connector) => {
    setConnectingId(connector.uid);
    try {
      await connect({ connector });
      onClose();
    } catch {
      setConnectingId(null);
    }
  };

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
          <h2 className="text-lg font-bold text-gray-900">Connect Wallet</h2>
          <button
            onClick={onClose}
            className="rounded-full p-2 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600"
            aria-label="Close"
          >
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        </div>

        <div className="px-6 py-4">
          {/* Education toggle */}
          {!showEducation && (
            <button
              onClick={() => setShowEducation(true)}
              className="mb-4 flex w-full items-center gap-2 rounded-xl bg-blue-50 p-3 text-left text-sm text-blue-700 transition-colors hover:bg-blue-100"
            >
              <svg className="h-5 w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span className="font-medium">New to wallets? Learn how it works</span>
            </button>
          )}

          {/* Education section (expandable) */}
          {showEducation && (
            <div className="mb-4 space-y-3">
              {EDUCATION_STEPS.map((step, i) => (
                <div key={i} className="rounded-xl bg-gray-50 p-3">
                  <p className="text-sm font-semibold text-gray-800">{step.title}</p>
                  <p className="mt-1 text-xs leading-relaxed text-gray-600">{step.body}</p>
                </div>
              ))}
              <button
                onClick={() => setShowEducation(false)}
                className="text-sm font-medium text-blue-600 hover:text-blue-700"
              >
                Got it, show me the options
              </button>
            </div>
          )}

          {/* Connector list */}
          <div className="space-y-2">
            {connectors.map((connector: Connector) => {
              const meta = getConnectorMeta(connector);
              const isConnecting = connectingId === connector.uid;

              return (
                <button
                  key={connector.uid}
                  onClick={() => handleConnect(connector)}
                  disabled={isPending}
                  className={`flex w-full items-center gap-3 rounded-xl border p-4 text-left transition-all ${
                    isConnecting
                      ? "border-red-200 bg-red-50"
                      : "border-gray-100 hover:border-gray-200 hover:bg-gray-50"
                  } ${meta.tag ? "ring-2 ring-red-100" : ""}`}
                >
                  {/* Icon */}
                  <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gray-100 text-lg font-bold text-gray-600">
                    {connector.icon ? (
                      <img src={connector.icon} alt="" className="h-6 w-6" />
                    ) : (
                      meta.icon
                    )}
                  </div>

                  {/* Labels */}
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-gray-900">
                        {meta.label}
                      </span>
                      {meta.tag && (
                        <span className="rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-bold uppercase text-red-700">
                          {meta.tag}
                        </span>
                      )}
                    </div>
                    <p className="mt-0.5 text-xs text-gray-500">
                      {isConnecting ? "Connecting..." : meta.description}
                    </p>
                  </div>

                  {/* Loading spinner */}
                  {isConnecting && (
                    <svg
                      className="h-5 w-5 animate-spin text-red-500"
                      fill="none"
                      viewBox="0 0 24 24"
                    >
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                    </svg>
                  )}
                </button>
              );
            })}
          </div>

          {/* Error */}
          {error && (
            <div className="mt-3 rounded-xl bg-red-50 p-3 text-sm text-red-700">
              {error.message.includes("rejected")
                ? "Connection was cancelled."
                : "Failed to connect. Please try again."}
            </div>
          )}
        </div>

        {/* Footer */}
        <div className="border-t border-gray-100 px-6 py-3">
          <p className="text-center text-xs text-gray-400">
            By connecting, you agree to our Terms of Service
          </p>
        </div>
      </div>
    </div>
  );
}
