  ...
  ({ product, selectedVariant }) => {
    ...
    const isSwapSupported = useCallback(async () => {
      ...
      try {
        const fromToken = TOKENS.find(
          (t) => t.symbol === walletSelectedToken.symbol
        );
    ...
    const validateSwapRequirements = useCallback(async () => {
      ...
      try {
        ...
        if (!balance || balance.numeric < computedTotals.totalInSelected) {
          updateState({
            purchaseError: `Insufficient ${walletSelectedToken.symbol} balance for swap`,
          });
          return false;
        }
        ...
        if (!pairSupported) {
          updateState({
            purchaseError: `${walletSelectedToken.symbol}/${product.paymentToken} swap not supported`,
          });
          return false;
        }
    ...
    const handleButtonClick = useCallback(async () => {
      ...
      if (!hasSufficientBalance) {
        updateState({
          purchaseError: `Insufficient ${walletSelectedToken.symbol} balance`,
        });
        return;
      }
    ...
    const handleConfirmSwap = useCallback(async () => {
      ...
      try {
        ...
        const targetToken = TOKENS.find(
          (t: StableToken) => t.symbol === product.paymentToken
        );
        if (targetToken) {
    ...
    const isLoading = state.isProcessing || isSwapping;
      ...
      <>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="text-white font-mono font-medium flex items-center gap-2">
                  {isLoadingBalance ? (
                    ...
                    formatBalance(
                      getBalance(walletSelectedToken.symbol)?.formatted
                    )
                  )}
                </div>
              </div>
        ...
        {state.showSwapModal && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
            <div className="bg-[#212428] rounded-lg p-6 max-w-sm w-full mx-4 space-y-4">
