...
const CustomerReviews = ({ productId, reviewcount }: CustomerReviewsProps) => {
  ...
  return (
    <div className="text-gray-400 px-4 sm:px-6 py-4">
        ...
        <>
          ...
          {visibleReviews < formattedReviews.length && (
            <div className="mt-4 text-center">
              <button
                className="bg-[#292B30] text-Red hover:bg-[#31333a] px-4 py-2 rounded-lg text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-Red/30"
                onClick={loadMoreReviews}
              >
                View all {formattedReviews.length} reviews
              </button>
            </div>
          )}
