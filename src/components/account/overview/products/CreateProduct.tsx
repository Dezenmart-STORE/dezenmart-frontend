...
const CreateProduct: React.FC<CreateProductProps> = ({ onProductCreated }) => {
  ...
  const handleUSDTChange = useCallback(
    (value: string) => {
      ...
      if (!isNaN(numValue)) {
        const fiatValue = convertPrice(numValue, "USD", fiatCurrency);
  ...
  const handleFiatChange = useCallback(
    (value: string) => {
      ...
      const numValue = parseFloat(value);
      if (!isNaN(numValue)) {
        const usdtValue = convertPrice(numValue, fiatCurrency, "USD");
  ...
  const validateForm = useCallback(() => {
    ...
    if (!paymentToken && !selectedToken?.symbol) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const matchedToken = availableTokens.find(
      ...
      if (matchedToken && chainId) {
        ...
        if (tokenAddress) {
          ...
          const tradeParams = buildTradeParams(
      ...
      if (selectedLogistics.length > 0) {
        ...
        formData.append(
          "logisticsProviders",
          "0x0c9db90a95a78bf6d9b2448fde00210f36ba61e4"
        );
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="text-lg">
                    {(() => {
                      const tokenIcon = availableTokens.find((t) => t.symbol === paymentToken)?.icon;
                      return typeof tokenIcon === "string" && tokenIcon ? (
                        <img
                          src={tokenIcon}
                          alt={paymentToken}
          ...
          <div className="pt-4">
            ...
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              <Button
                ...
                type="submit"
                disabled={!isConnected || isSubmitting || loading}
                iconPosition="start"
                icon={
                  isSubmitting ? (
